/*
 * $Id: cdfNativeLibrary.c,v 1.1.1.1 2022/04/01 12:25:56 mhliu Exp $
 */
/******************************************************************************
...
//printf("in JNI... ");
//for (iu=0; iu < arg->length; ++iu) printf("%02X ",*(((char *)arg->ptr)+iu));
//printf("\n");
...
//printf("###### from CDF: value=%s (len=%d)\n",(char *)arg.ptr,(int)arg.length); fflush(stdout);
	    if (realChar == 0) {
	      jobject bb = (*env)->NewDirectByteBuffer(env, (void *) arg.ptr,
						       (jlong) arg.length-1);
	      jclass cls_Charset = (*env)->FindClass(env, "java/nio/charset/Charset");
	      jmethodID mid_Charset_forName = (*env)->GetStaticMethodID(env, 
				cls_Charset, "forName",
				"(Ljava/lang/String;)Ljava/nio/charset/Charset;");
	      jobject charset = (*env)->CallStaticObjectMethod(env, cls_Charset,
				mid_Charset_forName, (*env)->NewStringUTF(env,
									  "UTF-8"));
	      jmethodID mid_Charset_decode = (*env)->GetMethodID(env, cls_Charset,
				"decode", "(Ljava/nio/ByteBuffer;)Ljava/nio/CharBuffer;");
	      jobject cb = (*env)->CallObjectMethod(env, charset, 
					  	    mid_Charset_decode, bb);
	      jclass cls_CharBuffer = (*env)->FindClass(env, "java/nio/CharBuffer");
	      jmethodID mid_CharBuffer_toString = (*env)->GetMethodID(env,
				cls_CharBuffer, "toString", "()Ljava/lang/String;");
	      jstring str = (*env)->CallObjectMethod(env, cb,
						     mid_CharBuffer_toString);
              (*env)->SetObjectField(env, arg.myObject, arg.id, str);
	    } else
	...
	case GET_:
		...
		while (item < NULL_) {
		  ...
		  switch(item) {
		    ...
		    case zVAR_PADVALUE_:
			if (item == zVAR_PADVALUE_) realChar = 1;
			nextItem = (*env)->CallObjectMethod(env, cmdVector, vElemID, i++);
			status = getItem(env, theCDF, nextItem, obj, &arg);
