...
C-----------------------------------------------------------------------
	...
	INTEGER*4 ID, STATUS, ENCODING, MAJORITY, NUM_DIMS,
     ...
     .		  cReserveOUT, Flag, lastUpdated, lastUpdatedO
...
C-----------------------------------------------------------------------
C Check leap second last upadted.
C-----------------------------------------------------------------------

	STATUS = CDF_LIB (GET_, CDF_LEAPSECONDLASTUPDATED_, LASTUPDATED,
     .                    NULL_, STATUS);
	IF (STATUS < CDF_OK) CALL QUIT_CDF (STATUS, "2.7");
C	IF (LASTUPDATED .NE. LASTUPDATEDO)
C     .    CALL QUIT_CDF(STATUS, "2.7a");
...
C    .		             PUT_, CDF_ENCODING_, ENCODING,
     .			     PUT_, CDF_MAJORITY_, MAJORITY,
C    .			           CDF_COMPRESSION_, cType, cParms,
     .		             NULL_, STATUS)
...
C       IF (cTypeOUT .NE. cType) CALL QUIT_CDF (STATUS, '16.8')
...
C-----------------------------------------------------------------------
	...
	IF (cTypeOUT .NE. NO_COMPRESSION) CALL QUIT_CDF (STATUS, '29.1')
