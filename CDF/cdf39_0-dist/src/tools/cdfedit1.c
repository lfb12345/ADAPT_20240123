...
Logical EditAttrs (G, CDFname, useFormat)
Logical G;                      /* If TRUE, global scoped attributes. */
char *CDFname;                  /* Name of the CDF being edited. */
Logical useFormat;              /* If TRUE, use variable attribute FORMAT to
                                   encode attribute value. */
{
   CDFstatus status; int attrX, entryX, entryType = 0;
   long attrN = 0, entryN = 0, fieldN = 0, *nEntries, *attrNs, **entryNs;
...
"Exit:   ________ Toggle scope:     ________")
   ...
   static struct ItemWindowStruct IW = {
      0, 0, 80, label, 2, header, 0, NULL, 0, NULL, NULL, NULL, NULL, 0, 0,
      NULL, NULL, REFRESHkey_FSI, TRUE, NSkey_FSI, PSkey_FSI
   };
   ...
   ***************************************************************************/
   if (!BuildAttrMenu(G,CDFname,&nAttrs,&attrNs,
		      &nEntries,&entryNs,&IW,useFormat)) return FALSE;
	 ...
	 *********************************************************************/
	 switch (fieldN) {
	   ...
	   *******************************************************************/
	   case 3: {
	     if (browseOnly)
	       ...
	       if (!EditEntry(attrN,entryN,entryType,TRUE,&changed,useFormat)) {
		 ItemWindow (DELETEiw, &IW);
		 return FALSE;
	       }
	       if (changed) {
	   ...
	   *******************************************************************/
	   case 4: {
	     Logical changed = FALSE;
	     if (!EditEntry(attrN,entryN,entryType,FALSE,&changed,useFormat)) {
	       ItemWindow (DELETEiw, &IW);
	       return FALSE;
	     }
	     if (changed) {
	 ...
	 if (!CreateEntry(G,attrN,&changed,useFormat)) {
	   ItemWindow (DELETEiw, &IW);
	   return FALSE;
	 }
	 if (changed) {
...
Logical BuildAttrMenu (G, CDFname, nAttrs, attrNs, nEntries, entryNs, IW, useFormat)
Logical G;
char *CDFname;
long *nAttrs;
long **attrNs;
...
Logical useFormat;
{
   ...
   int nLines, nItems;
   long attrN;
   long estLines = 0;          /* Estimated number of lines for the menu. */
   int style;
   char *format;
   ...
   ***************************************************************************/
   ...
   for (attrX = 0; attrX < *nAttrs; attrX++) {
	...
	CatNcharacters (IW->iLines[lineN], ATTRNAME_FIELD_LEN - strlen(field),
      ...
      IW->ibLens[curItem] = strlen(field);
	...
	for (entryX = 0; entryX < (*nEntries)[attrX]; entryX++) {
	   ...
	   *******************************************************************/
	   ...
	   status = CDFlib (GET_, ENTRY_DATA(entryType), binary,
			    NULL_);
	   if (!ReportStatus(status,FALSE)) return FALSE;
	   if (STRINGdataType(dataType)) *((char *)binary + numElems) = '\0';
           ...
           format = NULL;
	   if (G || !useFormat || STRINGdataType(dataType)) {
	     if (STRINGdataType(dataType)) {
	     int cnt; unsigned char *chkStr;
	     chkStr =  UTF8Check ((unsigned char *)binary, (int)numElems, &cnt);
	     if (chkStr == NULL)
	       n = EncodeValuesFormat (dataType, numElems, binary,
				       &(IW->iLines[lineN][curCol]), NULL, 0,
				       BOO(G,gAttrENTRYVALUE_FIELD_LENx,
					     vAttrENTRYVALUE_FIELD_LENx), style,
				       (size_t) 80-strlen(IW->iLines[lineN]));
	     else {
	       char *badStr;
	       badStr = malloc((size_t)numElems + 1);
	       memset (badStr, '.', (size_t)numElems);
	       badStr[(int)numElems] = '\0';
               n = EncodeValuesFormat (dataType, numElems, badStr,
				       &(IW->iLines[lineN][curCol]), NULL, 0,
				       BOO(G,gAttrENTRYVALUE_FIELD_LENx,
			 		     vAttrENTRYVALUE_FIELD_LENx), style,
				       (size_t) 80-strlen(IW->iLines[lineN]));
	     ...
	     n = EncodeValuesFormat (dataType, numElems, binary,
				     ...
				     BOO(G,gAttrENTRYVALUE_FIELD_LEN,
				           vAttrENTRYVALUE_FIELD_LEN), style,
				     (size_t) 80-strlen(IW->iLines[lineN]));
	   } else {
	     Logical toUseFormat;
	     toUseFormat = LogicalFALSE;
	     status = GetFORMATEntry (id, (entryType==zENTRYt),
				      (*entryNs)[attrX][entryX], &format);
	     if (status == CDF_OK) toUseFormat = LogicalTRUE;
	     if (toUseFormat == LogicalFALSE)
	       n = EncodeValuesFormat (dataType, numElems, binary,
				       &(IW->iLines[lineN][curCol]), NULL, 0,
				       vAttrENTRYVALUE_FIELD_LEN, style,
				       (size_t) 80-strlen(IW->iLines[lineN]));
	       ...
	       n = EncodeValuesFormat (dataType, numElems, binary,
				       &(IW->iLines[lineN][curCol]), format,
				       -1, vAttrENTRYVALUE_FIELD_LEN, style,
				       (size_t) 80-strlen(IW->iLines[lineN]));
	   }
	   ...
	   *******************************************************************/
	   ...
	   IW->iLens[curItem] = 
	       UTF8StrChars (&(IW->iLines[lineN][dataSpecCol]), 0);
	   IW->ibLens[curItem] = (int) strlen(&(IW->iLines[lineN][dataSpecCol])); 
	   curItem++;
	   /*******************************************************************
	   ...
	   *******************************************************************/
	   ...
	   IW->iCols[curItem] = curCol;
	   IW->iLens[curItem] = n;
	   IW->ibLens[curItem] = (int) strlen(&(IW->iLines[lineN][curCol]));
	   curItem++;
	   lineN++;
	}
      }
...
Logical EditEntry (attrN, entryN, entryType, newDataSpec, changed, useFormat)
long attrN;
long entryN;
int entryType;
Logical newDataSpec;
...
Logical useFormat;
{
  ...
  ****************************************************************************/
  if (!browseOnly && newDataSpec) {
    ...
    strcpyX (value, BOO(STRINGdataType(dataType),"\"\"",""),
	     MAX_ENTRYSTRING_LEN);
    if (STRINGdataType(dataType)) n = 2;
    else n = 0;
  }
  else {
    void *binary;
    ...
    format = NULL;
    if (entryType == gENTRYt || !useFormat || STRINGdataType(dataType)) {
      if (STRINGdataType(dataType)) {
        int cnt; unsigned char *chkStr;
        chkStr =  UTF8Check ((unsigned char *)binary, (int)numElems, &cnt);
        if (chkStr == NULL)
          n = EncodeValuesFormat (dataType, numElems, binary, value,
                                  NULL, 0, MAX_ENTRYSTRING_LEN, style,
                                  (size_t) sizeof(value));
        else {
          char *badStr;
          badStr = malloc((size_t)numElems + 1);
          memset (badStr, '.', (size_t)numElems);
          badStr[(int)numElems] = '\0';
          n = EncodeValuesFormat (dataType, numElems, badStr, value,
                                  NULL, 0, MAX_ENTRYSTRING_LEN, style,
                                  (size_t) sizeof(value));
          free (badStr);
          badStr = NULL;
      ...
      Logical toUseFormat;
      toUseFormat = LogicalFALSE;
      status = GetFORMATEntry (id, (entryType==zENTRYt), entryN, &format);
      if (status == CDF_OK) toUseFormat = LogicalTRUE;
      if (!toUseFormat) 
	n = EncodeValuesFormat (dataType, numElems, binary, value, NULL, 0,
				MAX_ENTRYSTRING_LEN, style,
				(size_t) sizeof(value));
      else
	n = EncodeValuesFormat (dataType, numElems, binary, value, format,
				-1, MAX_ENTRYSTRING_LEN, style,
				(size_t) sizeof(value));
    }
    cdf_FreeMemory (binary, FatalError);
    if (format != NULL) cdf_FreeMemory (format, FatalError);
  ...
  ****************************************************************************/
  ...
  cursorAtThisChar = BOO(browseOnly,0,n - BOO(STRINGdataType(dataType),1,0));
  ...
  for (;;) {
     if (EnterPW(&PW,ENTRYVALUEhelpID)) {
       ...
       if (DecodeValues(PW.value,dataType,&newNumElems,
                        &newBinary,style)) {
         if (!STRINGdataType(dataType) || newNumElems > 0) {
	   status = CDFlib (SELECT_, ATTR_, attrN,
				     ENTRY(entryType), entryN,
	 ...
	 if (NoMoreAccess(NULL)) {
	   ...
	   ProblemWindow ("Error decoding values. " \
			  "Use 'yyyy-mm-ddThh:mm:ss.sss' epoch format.", FALSE);
         else if (dataType == CDF_EPOCH16)
	   ProblemWindow ("Error decoding values. " \
			  "Use 'yyyy-mm-ddThh:mm:ss.mmm.uuu.nnn.ppp' epoch format.",
			  FALSE);
         else if (dataType == CDF_TIME_TT2000)
	   ProblemWindow ("Error decoding values. " \
			  "Use 'yyyy-mm-ddThh:mm:ss.mmmuuunnn' epoch format.",
			  FALSE);
         else
...
Logical EditVarEntries (Z, varN, nVars, useFormat)
Logical Z;
long varN;
long nVars;
Logical useFormat;
{
   ...
   ***************************************************************************/
   if (!BuildVarEntryMenu(Z,varN,&nAttrs,&attrNs,&entryIs,&IW,useFormat))
     return FALSE;
   ItemWindow (NEWiw, &IW, 0);
   /***************************************************************************
   * Read/process keystrokes until request to exit menu.
	 ...
	 *********************************************************************/
	 switch (fieldN) {
		  ...
		  if (EnterPW(&PW,ATTRRENAMEhelpID)) {
		    ...
		    if (DecodeDelimitedString(PW.value,attrName)) {
		      ...
		      if (ReportStatus(status,FALSE)) {
		        ...
		        if (!BuildVarEntryMenu(Z,varN,&nAttrs,&attrNs,
					       &entryIs,&IW,useFormat)) {
			  PromptWindow (DELETEpw, &PW);
			  ItemWindow (DELETEiw, &IW);
			  return FALSE;
			}
	   ...
	   *******************************************************************/
	   ...
	   /*******************************************************************
	   * Modify value(s). Editing the entry value
	   *******************************************************************/
	   case 3: {
	     Logical changed = FALSE;
	     if (!EditEntry(attrN,varN,
			    BOO(Z,zENTRYt,rENTRYt),
			    FALSE,&changed,useFormat)) {
	       ItemWindow (DELETEiw, &IW);
	       return FALSE;
	     }
	     if (changed) {
...
Logical BuildVarEntryMenu (Z, varN, nAttrs, attrNs, entryIs, IW, useFormat)
Logical Z;
long varN;
long *nAttrs;
long **attrNs;
...
{
   CDFstatus status;
   int attrX, curItem, curCol, lineN, nItems, n, dataSpecCol;
   ...
   *attrNs = (long *) cdf_AllocateMemory ((size_t) (*nAttrs * sizeof(long)),
				      FatalError);
   *entryIs = (Logical *) cdf_AllocateMemory ((size_t) (*nAttrs *
							 sizeof(Logical)),
					      FatalError);
   ...
   ***************************************************************************/
   ...
   for (attrX = 0; attrX < *nAttrs; attrX++) {
	...
	binary = cdf_AllocateMemory ((size_t) ((numElems+
					       (STRINGdataType(dataType)?1:0))* 
				     CDFelemSize(dataType)), FatalError);
	...
	format = NULL;
	if (!useFormat || STRINGdataType(dataType))
	  n = EncodeValuesFormat (dataType, numElems, binary,
				  &(IW->iLines[lineN][curCol]), NULL, 0,
	  ...
	  Logical toUseFormat;
	  toUseFormat = LogicalFALSE;
	  status = GetFORMATEntry (id, Z, varN, &format);
	  if (status == CDF_OK) toUseFormat = LogicalTRUE;
	  if (!toUseFormat)
	    n = EncodeValuesFormat (dataType, numElems, binary,
				    &(IW->iLines[lineN][curCol]), NULL, 0,
	    ...
	    n = EncodeValuesFormat (dataType, numElems, binary,
				    &(IW->iLines[lineN][curCol]), format,
				    -1, varENTRYVALUE_FIELD_LEN, style,
				    (size_t) 80-strlen(IW->iLines[lineN]));
	}
	cdf_FreeMemory (binary, FatalError);
	if (format != NULL) cdf_FreeMemory (format, FatalError);
	IW->iLineNs[curItem] = lineN;
	IW->iCols[curItem] = dataSpecCol;
	IW->iLens[curItem] = UTF8StrChars (&(IW->iLines[lineN][dataSpecCol]),
					   0);
	IW->ibLens[curItem] = (int) strlen(&(IW->iLines[lineN][dataSpecCol]));
	curItem++;
	IW->iLineNs[curItem] = lineN;
...
Logical CreateEntry (G, attrN, changed, useFormat)
Logical G;
long attrN;
...
Logical useFormat;
{
  ...
  ****************************************************************************/
  ...
  if (G) {
    ...
    **************************************************************************/
    ...
    static char label[15+CDF_ATTR_NAME_LEN256+1];
    static struct ItemWindowStruct IW = {
	4, 16, 48, label, 1, header, 0, NULL, 0, NULL, NULL, NULL, NULL, 7,
    ...
    for (varX = 0; varX < nVars; varX++) {
       ...
       IW.ibLens[varX] = (int) MINIMUM(40,lengthX);
    }
    ItemWindow (NEWiw, &IW, 0);
  ...
  * Create the entry.
  ****************************************************************************/
  return EditEntry(attrN,entryN,entryType,TRUE,changed,useFormat);
}
...
Logical EditAttrsX (G, CDFname, useFormat)
Logical G;                      /* If TRUE, global scoped attributes. */
char *CDFname;                  /* Name of the CDF being edited. */
Logical useFormat;              /* If TRUE, use FORMAT attribute to encode
                                   value. */
{
   CDFstatus status;
   ...
   ***************************************************************************/
   if (!BuildAttrMenuX(G,CDFname,&nAttrs,
		       &nAttrsOfScope,&attrNs,&IW,useFormat)) return FALSE;
   ItemWindow (NEWiw, &IW, 0);
   /***************************************************************************
   * Read/process keystrokes until request to exit menu.
   ***************************************************************************/
	 ...
	 switch (fieldN) {
		  ...
		  if (EnterPW(&PWt,ATTRRENAMEhelpID)) {
		    ...
		    if (DecodeDelimitedString(PWt.value,attrName)) {
		      ...
		      if (ReportStatus(status,FALSE)) {
		        ...
		        if (!BuildAttrMenuX(G,CDFname,&nAttrs,
					    &nAttrsOfScope,&attrNs,&IW,useFormat)) {
			  PromptWindow (DELETEpw, &PWt);
			  ItemWindow (DELETEiw, &IW);
			  return FALSE;
			}
	   ...
	   *******************************************************************/
	   case 1: {
	     ...
	     if (!EditAttrEntriesX(G,attrN,nAttrs,useFormat)) {
	       ItemWindow (DELETEiw, &IW);
	       return FALSE;
	     }
...
Logical BuildAttrMenuX (G, CDFname, nAttrs, nAttrsOfScope, attrNs, IW, useFormat)
Logical G;
char *CDFname;
long *nAttrs;
long *nAttrsOfScope;
...
Logical useFormat;
{
   ...
   ***************************************************************************/
	...
	**********************************************************************/
	...
	IW->ibLens[itemN] = MinInt(1 + nameLen + 1,attrNameFieldLen);
	/**********************************************************************
	...
	**********************************************************************/
	...
	IW->ibLens[itemN+1] = BOO(browseOnly,16,14);
	/**********************************************************************
	...
	**********************************************************************/
	...
	IW->ibLens[itemN+2] = BOO(browseOnly,20,18);
	/**********************************************************************
  ...
  ****************************************************************************/
  ...
  for (entryN = 0; entryN <= maxEntry; entryN++) {
	 ...
	 status = CDFlib (GET_, gENTRY_DATA_, line,
			  NULL_);
	 if (!ReportStatus(status,FALSE)) return FALSE;
	 if (STRINGdataType(dataType)) line[(int)numElems] = NUL;
	 if (STRINGdataType(dataType)) {
	   int cnt;
	   unsigned char *chkStr;
	   chkStr =  UTF8Check ((unsigned char *)line, (int)numElems, &cnt);
	   if (chkStr == NULL)
	     strcatX (EW->eText, line, (size_t) totalBytes);
             ...
             memset (badStr, '.', (size_t)numElems);
             badStr[(int)numElems] = '\0';
             strcatX (EW->eText, badStr, (size_t) totalBytes);
             free (badStr);
             badStr = NULL;
	   }
	 }
  ...
  for (;;) {
     ...
     *************************************************************************/
     ...
     if (length > 0) {
       ...
       empty[0] = (char) '\0';
       status = CDFlib (SELECT_, gENTRY_, entryN,
			PUT_, gENTRY_DATA_, CDF_CHAR, 1L, empty,
			NULL_);
       switch (status) {
	 case NO_SUCH_ENTRY:
	   break;
...
Logical EditAttrEntriesX (G, attrN, nAttrs, useFormat)
...
Logical useFormat;
{
   ...
   long nEntries; long *entryNs; int *entryTypes, entryType = 0;
   static Logical first = TRUE;
   /***************************************************************************
   * First time...
   ***************************************************************************/
   ...
   ***************************************************************************/
   if (!BuildAttrEntriesMenuX(G,attrN,&nEntries,
			      &entryNs,&entryTypes,&IW,useFormat)) return FALSE;
   ItemWindow (NEWiw, &IW, 0);
   /***************************************************************************
   * Read/process keystrokes until request to exit menu.
   ***************************************************************************/
	   ...
	   case 2: {
		 ...
		 return FALSE;
	       ...
	       if (changed) {
	         FreeAttrEntriesMenuX (entryNs, entryTypes, &IW);
	         if (!BuildAttrEntriesMenuX(G,attrN,&nEntries,
					    &entryNs,&entryTypes,&IW,useFormat)) {
		   ItemWindow (DELETEiw, &IW);
		   return FALSE;
		 }
	         ItemWindow (UPDATEiw, &IW, IW.itemN);
...
Logical BuildAttrEntriesMenuX (G, attrN, nEntries, entryNs, entryTypes, IW, useFormat)
Logical G;
long attrN;
long *nEntries;
long **entryNs;
...
Logical useFormat;
{
   ...
   ***************************************************************************/
   for (entryX = 0, curCol = 0; entryX < *nEntries; entryX++, curCol = 0) {
      ...
      varNum = (*entryNs)[entryX];
      status = CDFlib (SELECT_, ATTR_, attrN,
				ENTRY(entryType), varNum,
		       GET_, ENTRY_DATATYPE(entryType), &dataType,
			     ENTRY_NUMELEMS(entryType), &numElems,
		       NULL_);
      if (!ReportStatus(status,FALSE)) return FALSE;
	...
	if (!toUseFormat)
	  ...
	  n = EncodeValuesFormat (dataType, numElems, binary,
				  &(IW->iLines[entryX][curCol]), format,
				  -1, vAttrENTRYVALUE_FIELD_LENx, style,
				  (size_t) 80-strlen(IW->iLines[entryX]));
      }
      cdf_FreeMemory (binary, FatalError);
      if (format != NULL) cdf_FreeMemory (format, FatalError);
      ...
      *******************************************************************/
      ...
      IW->iLens[itemN] = UTF8StrChars (&(IW->iLines[entryX][dataSpecCol]),
				       0);
      IW->ibLens[itemN] = (int) strlen(&(IW->iLines[entryX][dataSpecCol]));
      /*******************************************************************
      * Set `value' item control information.
      *******************************************************************/
      itemN = (4 * entryX) + 3;
      ...
      IW->iCols[itemN] = curCol;
      IW->iLens[itemN] = n;
      IW->ibLens[itemN] = (int) strlen (&(IW->iLines[entryX][curCol]));
   }
...
void FillPadValue (dataType, text)
long dataType;
char *text;
{
  char string[] = "\"\"                                                      ";
  if (dataType == CDF_BYTE || dataType == CDF_INT1)
    sprintf(text, "%d", DEFAULT_BYTE_PADVALUE);
  else if (dataType == CDF_UINT1)
    sprintf(text, "%d", DEFAULT_UINT1_PADVALUE);
  else if (dataType == CDF_INT2)
    sprintf(text, "%hd", DEFAULT_INT2_PADVALUE);
  else if (dataType == CDF_UINT2)
    sprintf(text, "%uh", DEFAULT_UINT2_PADVALUE);
  else if (dataType == CDF_INT4)
    sprintf(text, Int32FORMAT, DEFAULT_INT4_PADVALUE);
  else if (dataType == CDF_UINT4)
    sprintf(text, Int32uFORMAT, DEFAULT_UINT4_PADVALUE);
  else if (dataType == CDF_INT8)
    sprintf(text, Int64FORMAT, DEFAULT_INT8_PADVALUE);
  else if (FLOAT4dataType(dataType))
    sprintf(text, "%g", DEFAULT_FLOAT_PADVALUE);
  else if (DOUBLEdataType(dataType))
    sprintf(text, "%g", DEFAULT_DOUBLE_PADVALUE);
  else if (dataType == CDF_EPOCH)
    sprintf(text, "%s", "0000-01-01T00:00:00.000");
  else if (dataType == CDF_EPOCH16)
    sprintf(text, "%s", "0000-01-01T00:00:00.000000000000");
  else if (dataType == CDF_TIME_TT2000)
    sprintf(text, "%s", "0000-01-01T00:00:00.000000000");
