...
char *argV[];
{
  ...
  static char *validQuals[] = {
    ...
    "tolerance", "format", "noformat", "data", "nodata", "ziso8601",
    "novalidate", NULL
  };
  ...
  ****************************************************************************/
  switch (argC) {
	...
	report[INFOs] = REPORTinfosDEFAULT;
      ...
      * `neg2posfp0', `location', `value', `statistics', `format', `nodata'
      * qualifiers.
      ************************************************************************/
      ...
      qopError = qopError | !TFqualifier (qop,&format,FORMATqual,
					  ...
					  "format");
      qopError = qopError | !TFqualifier (qop,&data,DATAqual,
					  NODATAqual,DEFAULTdataCMP,
					  "data");
      /************************************************************************
...
Logical CompareGeneral ()
{
  ...
  ****************************************************************************/
  ...
  if (cmpEtc && (CDFcPct1 != CDFcPct2)) {
    diffFound = TRUE;
    snprintf (oText, (size_t) sizeof(oText),
	      "DIFFERENT CDF compression rate (compressed/uncompressed) (%ld%% vs. %ld%%)",
	      CDFcPct1, CDFcPct2);
    OutputWithMargin (stdout, oText, MAX_SCREENLINE_LEN, 4);
  }
  /****************************************************************************
  ...
  ****************************************************************************/
  if (cmpEtc && (!PriorTo("3.3.2",(Int32)version1, (Int32)release1,
                          (Int32)increment1) &&
                 !PriorTo("3.3.2",(Int32)version2, (Int32)release2,
                          (Int32)increment2)) &&
      (tt2000updated1 != tt2000updated2)) {
    diffFound = TRUE;
...
int entryType;
{
  ...
  long entryNumString1, entryNumString2;
  ...
  if ((entryType == rENTRYt || entryType == zENTRYt) &&
      (STRINGdataType(entryDataType1) && STRINGdataType(entryDataType2))) {
    status1 = CDFlib (SELECT_, CDF_, id1,
                      CONFIRM_, ENTRY_EXISTENCE(entryType), entryN1,
                      SELECT_, ENTRY(entryType), entryN1,
                      GET_, ENTRY_NUMSTRINGS(entryType==zENTRYt),
                            &entryNumString1,
                      NULL_);
    status2 = CDFlib (SELECT_, CDF_, id2,
                      CONFIRM_, ENTRY_EXISTENCE(entryType), entryN2,
                      ...
                      GET_, ENTRY_NUMSTRINGS(entryType==zENTRYt),
                            &entryNumString2,
                      NULL_);
    if (entryNumString1 != entryNumString2 && ((entryNumString1 > 1) ||
                                               (entryNumString2 > 1))) {
      diffFound = TRUE;
      snprintf (oText, (size_t) sizeof(oText),
                "DIFFERENT number of strings for %s number %s",
                e, entryNumField);
      snprintf (EofS(oText), (size_t) sizeof(oText)-strlen(oText),
                "%s of attribute \"%s\" (%ld vs. %ld)",
                varNameField, attrName, entryNumString1, entryNumString2);
      OutputWithMargin (stdout, oText, MAX_SCREENLINE_LEN, 4);
    }
  }
  ...
  if (STRINGdataType(entryDataType1)) ++Nbytes;
  ...
  if (STRINGdataType(entryDataType1)) {
    *((char *)entryValue1+entryNumElems1) = (char) '\0';
    *((char *)entryValue2+entryNumElems1) = (char) '\0';
  }
  if (memcmp(entryValue1,entryValue2,(size_t)Nbytes) != 0) {
	...
	(entryDataType1 == CDF_INT8) || (entryDataType1 == CDF_TIME_TT2000)) {
      ...
      if ((!floatSet && FLOAT4dataType(entryDataType1) &&
	   (CheckNaN(entryValue1, entryValue2, entryDataType1) > 1)) ||
          (!doubleSet && FLOAT8dataType(entryDataType1) &&
	   (CheckNaN(entryValue1, entryValue2, entryDataType1) > 1)) ||
          (!doubleSet && entryDataType1 == CDF_EPOCH16) ||
          (floatSet && FLOAT4dataType(entryDataType1)) ||
          (doubleSet && (FLOAT8dataType(entryDataType1) ||
    ...
    if (comp) {
      ...
      if (displayValue) {
        ...
        WriteEntryValue (stdout, entryDataType1, entryNumElems1, entryValue1,
		         21, MAX_SCREENLINE_LEN-1, iso8601, NULL);
        WriteOut (stdout, "\n");
...
Logical Z;
{
  ...
  for (varN1 = 0; varN1 < BOO(Z,zNumVars1,rNumVars1); varN1++) {
	 ...
	 OutputWithMargin (stdout, oText, MAX_SCREENLINE_LEN, 2);
       ...
       if (data) cmpVarValues = TRUE;
       else cmpVarValues = FALSE;
	 ...
	 diffFound = TRUE;
	 strcpyX (oText, "DIFFERENT compression rate (compressed/uncompressed) for",
		  MAX_oTEXT_LEN);
	 snprintf (EofS(oText), (size_t) sizeof(oText)-strlen(oText),
		   " %s \"%s\" (%ld%% vs. %ld%%)",
	   ...
	   if (zNumDims1 > 0) {
		...
		if (zDimSizes1[i] != zDimSizes2[i]) {
		  ...
		  for (j = 0; j < zNumDims1; j++) {
		     snprintf (EofS(oText),
			       (size_t) sizeof(oText)-strlen(oText),
			       "%ld ", zDimSizes1[j]);
		  }
		  strcatX (oText, "vs.", MAX_oTEXT_LEN);
...
Logical Z;
{
  ...
  struct GroupStruct groups; struct HyperStruct hyperSt;
  Byte1 *value1, *value2, *buffer1, *buffer2, **handles[2];
  int dimN, dimNt;
  char *cformat1 = NULL, *cformat2 = NULL;
  ...
  ****************************************************************************/
  ...
  if (status1 == CDF_OK) {
    if (STRINGdataType(dataType)) {
      size_t iix; 
      cformat1 = (char *) cdf_AllocateMemory ((size_t) numElems + 1,
                                            FatalError);
      status1 = CDFlib (GET_, BOO(Z,zENTRY_DATA_,rENTRY_DATA_), cformat1,
                        NULL_);
      if (!StatusHandlerCmp("CDF1",status1)) return FALSE;
      if (STRINGdataType(dataType)) *((char *)cformat1+numElems) = '\0';
      iix = strlen((char *)cformat1);
      if (iix < (size_t)numElems)
        ((char *)cformat1)[(int)iix] = '\0';
      else
        ((char *)cformat1)[(int)numElems] = '\0';
    }
  }
  status2 = CDFlib (SELECT_, CDF_, id2,
			     VAR(Z), varN2,
  ...
  if (status2 == CDF_OK) {
    if (STRINGdataType(dataType)) {
      size_t iix;
      cformat2 = (char *) cdf_AllocateMemory ((size_t) numElems + 1,
                                            FatalError);
      status2 = CDFlib (GET_, BOO(Z,zENTRY_DATA_,rENTRY_DATA_), cformat2,
                        NULL_);
      if (!StatusHandlerCmp("CDF1",status2)) return FALSE;
      if (STRINGdataType(dataType)) *((char *)cformat2+numElems) = '\0';
      iix = strlen((char *)cformat2);
      if (iix < (size_t)numElems)
        ((char *)cformat2)[(int)iix] = '\0';
      else
        ((char *)cformat2)[(int)numElems] = '\0';
    }
  }
  /****************************************************************************
  * Compare pad values.
  ****************************************************************************/
  ...
  if (status1 != NO_PADVALUE_SPECIFIED) {
	...
	if ((varDataType1 == CDF_INT1) || (varDataType1 == CDF_INT2) ||
	  ...
	  if ((!floatSet && FLOAT4dataType(varDataType1) &&
	       (CheckNaN(padValue1, padValue2, varDataType1) > 1)) ||
	      (!doubleSet && FLOAT8dataType(varDataType1) &&
	       (CheckNaN(padValue1, padValue2, varDataType1) > 1)) ||
	      (!doubleSet && varDataType1 == CDF_EPOCH16) ||
	      (floatSet && FLOAT4dataType(varDataType1)) ||
	      (doubleSet && (FLOAT8dataType(varDataType1) ||
	...
	if (comp) {
	  ...
	  if (displayValue) {
	    ...
	    EncodeValuesFormat (varDataType1, varNumElems1, padValue1, oText,
	    		        NULL, 0, MAX_SCREENLINE_LEN - 19, iso8601,
				(size_t) sizeof(oText));
  ...
  ****************************************************************************/
  if (numDims < 2 || majority1 == majority2) {
    ...
    for (hyperN = 0; hyperN < nHypers; hyperN++) {
       status1 = HYPER_READ (id1, Z, hyperSt, buffer1);
       if (!StatusHandlerCmp("CDF1",status1)) return FALSE;
       if (pctLog) WriteOutPct (PCT(hyperN,nHypers,1,3));
       CHECKforABORTso
       status2 = HYPER_READ (id2, Z, hyperSt, buffer2);
       if (!StatusHandlerCmp("CDF2",status2)) return FALSE;
       ...
       if (status1 != status2) {
         if (status1 == VIRTUAL_RECORD_DATA) {
           snprintf (oText, (size_t) sizeof(oText),
                     ...
                     hyperSt.recNumber,hyperSt.recNumber+hyperSt.recCount-1,
		     varName);
           OutputWithMargin (stdout, oText, MAX_SCREENLINE_LEN, 4);
         }
         if (status2 == VIRTUAL_RECORD_DATA) {
           snprintf (oText, (size_t) sizeof(oText),
                     "Real vs Virtual in record range: %ld to %ld for \"%s\". ",
                     hyperSt.recNumber,hyperSt.recNumber+hyperSt.recCount-1,
		     varName);
           OutputWithMargin (stdout, oText, MAX_SCREENLINE_LEN, 4);
         }
	 ...
	 for (valueN = 0, value1 = buffer1, value2 = buffer2;
	   ...
	   if (memcmp(value1,value2,(size_t)NvalueBytes)) {
		 ...
		 (varDataType1 == CDF_INT4) || (varDataType1 == CDF_UINT1) ||
		 (varDataType1 == CDF_UINT2) || (varDataType1 == CDF_UINT4) ||
		 (varDataType1 == CDF_INT8) ||
                 (varDataType1 == CDF_TIME_TT2000)) {
	       comp = TRUE;
	     } else if (STRINGdataType(varDataType1)) {
	     ...
	     if (comp) {
	       AtRecordIndices (&hyperSt, valueN, rowMajor1, numDims,
		 ...
		 lastRecordDiff = recNumber;
	       ...
	       ReportValueDifference (recNumber, recVary, numDims, dimIndices,
				      ...
				      (format==TRUE?cformat1:NULL),
				      (format==TRUE?cformat2:NULL));
	     }
	   }
	 }
       }
       if (pctLog) WriteOutPct (PCT(hyperN,nHypers,3,3));
       IncrHyperParms (&hyperSt, &groups, numDims, rowMajor1, &nValues);
       CHECKforABORTso
    }
    cdf_FreeMemory (buffer1, FatalError);
    cdf_FreeMemory (buffer2, FatalError);
    if (cformat1 != NULL) cdf_FreeMemory (cformat1, FatalError);
    if (cformat2 != NULL) cdf_FreeMemory (cformat2, FatalError);
  ...
  ****************************************************************************/
  if (HyperFullRecord(&groups,numDims)) {
    ...
    InitHyperParms (&hyperSt, &groups, numDims, &nHypers, &nValues);
    for (hyperN = 0; hyperN < nHypers; hyperN++) {
       status2 = HYPER_READ (id2, Z, hyperSt, buffer1);
       if (!StatusHandlerCmp("CDF2",status2)) return FALSE;
  ...
  for (hyperN = 0; hyperN < nHypers; hyperN++) {
     ...
     status2 = CDFlib (SELECT_, CDF_, id2,
				BOO(Z,zVAR_RECNUMBER_,
				      rVARs_RECNUMBER_), hyperSt.recNumber,
		       NULL_);
     if (!StatusHandlerCmp("CDF2",status2)) return FALSE;
     for (dimN = 0; dimN < numDims; dimN++) {
	dimIndices[dimN] = hyperSt.dimIndices[dimN];
     }
	...
	if (memcmp(value1,value2,(size_t)NvalueBytes)) {
	  ...
	  if (comp) {
	    valueDiffCount++;
	    if (hyperSt.recNumber != lastRecordDiff) {
	      recordDiffCount++;
	      lastRecordDiff = hyperSt.recNumber;
	    }
	    ReportValueDifference (hyperSt.recNumber, recVary, numDims,
...
void AtRecordIndices (hyperSt, valueN, rowMajor, numDims, nValuesPerRec,
...
{
  int dimN;
  *recNumber = (valueN / nValuesPerRec) + hyperSt->recNumber;
...
char *format2;
{
  if (locations) {
    ...
    if (displayValue) {
      WriteOut (stdout, "        CDF1 value> ");
      if (FLOAT4dataType(varDataType1) &&
          *(float *)value1 <= DEFAULT_FLOAT_PADVALUE)
        EncodeValuesFormat (varDataType1, varNumElems1, value1, oText, NULL, 0,
			    MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      else if (DOUBLEdataType(varDataType1) &&
               *(double *)value1 <= DEFAULT_DOUBLE_PADVALUE)
        EncodeValuesFormat (varDataType1, varNumElems1, value1, oText, NULL, 0,
			    MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      else
        EncodeValuesFormat (varDataType1, varNumElems1, value1, oText, format1,
                            0, MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      WriteOut (stdout, oText);
      WriteOut (stdout, "\n");
      WriteOut (stdout, "        CDF2 value> ");
      if (FLOAT4dataType(varDataType2) &&
          *(float *)value2 <= DEFAULT_FLOAT_PADVALUE)
        EncodeValuesFormat (varDataType2, varNumElems2, value2, oText, NULL, 0,
			    MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      else if (DOUBLEdataType(varDataType2) &&
               *(double *)value2 <= DEFAULT_DOUBLE_PADVALUE)
        EncodeValuesFormat (varDataType2, varNumElems2, value2, oText, NULL, 0,
			    MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      else        
        EncodeValuesFormat (varDataType2, varNumElems2, value2, oText, format2,
                            0, MAX_SCREENLINE_LEN - 21, iso8601,
			    (size_t) sizeof(oText));
      WriteOut (stdout, oText);
      WriteOut (stdout, "\n");
...
double doubleValue;
{
  ...
  if (FLOAT4dataType(dataType)) {
     ...
     *************************************************************************/
     ...
     if (floatValue >= 0.0) {
       if (fabs(*((float *)value1) - *((float *)value2)) > fabs(floatValue)) 
	 return TRUE;
       else return FALSE;
     } else {
  ...
  } else if (FLOAT8dataType(dataType)) { 
     nanID = CheckNaN(value1, value2, dataType);
     if (nanID == 1) return FALSE;
     if (nanID == 2) return TRUE;
     if (doubleValue >= 0.0) {
       if (fabs(*((double *)value1) - *((double *)value2)) > fabs(doubleValue))
         return TRUE;
       else return FALSE;
     } else {
     ...
     nanID1 = CheckNaN((void *)&epoch16_a[0], (void *)&epoch16_b[0], dataType);
     nanID2 = CheckNaN((void *)&epoch16_a[1], (void *)&epoch16_b[1], dataType);
     if (nanID1 == 1 && nanID2 == 1) return FALSE;
     if (nanID1 == 2 || nanID2 == 2) return TRUE;
     if (doubleValue >= 0.0) {
       if ((fabs(epoch16_a[0] - epoch16_b[0]) > 
            fabs((doubleValue==0.0?DBL_EPSILON:doubleValue))) || 
  ...
  ****************************************************************************/
  ...
  for (;;) {
	...
	**********************************************************************/
	...
	int n;
	*argC = 1;
	*argV = (char **) cdf_AllocateMemory ((size_t)*argC * sizeof(char *), FatalError); 3115,
	n = 0;
	MAKEstrARGv (argV, n, pgmName)
#ifdef __MWERKS__
        DisposeRoutineDescriptor (FilterDialogQOPsoUPP);
