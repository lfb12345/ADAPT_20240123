...
void *buffer;
{
  ...
  *   2. Full physical records must be being read, and
  ...
  ****************************************************************************/
  ...
  if (fullPhyRec && 
    ...
    if (!sX(ReadVarValues64(CDF,Var,firstPhyRec,
			    INT32_ZERO,nValues,buffer),&pStatus))
      return pStatus;
    return pStatus;
  }
  /****************************************************************************
  ...
  ****************************************************************************/
  ...
  for (i = 0, recNum = rd->recNumber, tBuffer = buffer; i < rd->recCount;
     ...
     if (fullPhyRec) {
       ...
       * time from the file
       *******************************************************************/
       phyBuffer = (Byte1 *) cdf_AllocateMemory((size_t) Var->NphyRecBytes,
						 NULL);
       if (phyBuffer != NULL) {
    ...
    **************************************************************************/
	 ...
	 * Less than the full physical record is to be written.
	 ...
	 if (!sX(HyperWriteDim64(Var->numDims,Var->dimSizes,
         ...
         if (phyBuffer != NULL) {
           if (!sX(WriteVarValues64(CDF,Var,phyRecN,INT32_ZERO,
                                    (OFF_T) Var->NphyRecValues,phyBuffer),
                   &pStatus)) {
	     cdf_FreeMemory(phyBuffer,NULL);
	     return pStatus;
	   }
