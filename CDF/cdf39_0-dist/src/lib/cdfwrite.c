...
#endif
{
  ...
  for (;;) {
	 ...
	 if (value != NULL) {
	   if (STRINGdataType(AEDR->DataType)) {
	     ...
	     newValue = (char *) cdf_AllocateMemory (sizeof(char) *
      						     AEDR->NumElems+1, NULL);
	     memset ((void *)newValue, '\0', (size_t)AEDR->NumElems+1);
	 ...
	 if (STRINGdataType(dataType)) {
	   ...
	   if (!sX(WriteBuffer(CDF,fp,dataType,numElems,(void *)newValue),
		   &pStatus)) {
	     cdf_FreeMemory (newValue, NULL);
	     return pStatus;
	   } else
  ...
  for (;;) {
	 ...
	 if (zVar) {
	   if (!sX(ReadVDR(CDF,fp,offset,zVar,
			   VDR_zNUMDIMS,&nDims,
			   VDR_NULL),&pStatus)) return pStatus;
	   tOffset = offset + zVDR_DIMVARYS_OFFSETb + (nDims * 4);
	 }
	 else {
	   Int32 GDRoffset;
	   if (!sX(ReadCDR(fp,V2_CDR_OFFSET,
	 ...
	 if (zVar) {
	   ...
	   if (!sX(ReadGDR(fp,GDRoffset,
			   GDR_rNUMDIMS,&rNumDims,
			   GDR_NULL),&pStatus)) return pStatus;
	   tOffset = offset + rVDR_PADVALUE_OFFSETb + (rNumDims*4);
	 }
	 if (!SEEKv(fp,tOffset,vSEEK_SET)) return CWE;
	 if (STRINGdataType(dataType)) {
	   char *newPad; size_t len;
	   ...
	   memcpy ((void *)newPad, (void *)padValue,
		   (size_t)MINIMUM(numElems,len));
	   if (!sX(WriteBuffer(CDF,fp,dataType,numElems,(void *)newPad),
		   &pStatus)) {
	     cdf_FreeMemory (newPad, NULL);
	     return pStatus;
	   } else
	   ...
	   if (!sX(WriteBuffer(CDF,fp,dataType,numElems,padValue),
...
#endif
{
  ...
  for (;;) {
	 ...
	 switch (field) {
	   ...
	   case VXR_LASTREC: tOffset += nEntries * 4; break;
	   case VXR_OFFSET: tOffset += 2 * nEntries * 4; break;
	   default: return CDF_INTERNAL_ERROR;
	 }
	 if (!SEEKv(fp,tOffset,vSEEK_SET)) return CWE;
