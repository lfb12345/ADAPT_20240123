...
# endif
    ...
    **************************************************************************/
    CDF = (struct CDFstruct *) 
		cdf_AllocateMemory ((size_t)sizeof(struct CDFstruct), NULL);
    if (CDF == NULL) {
      V_close64 (dotFp, NULL, NULL);
      CDFdeleteFile (CDFnameX);
      return BAD_MALLOC;
    }
    CDF->CDFname = (char *) cdf_AllocateMemory ((size_t)strlen(CDFnameT) + 1,
						 NULL);
    if (CDF->CDFname == NULL) {
      V_close64 (dotFp, NULL, NULL);
      CDFdeleteFile (CDFnameX);
    ...
    CDF->GDRoffset64 = (OFF_T) NO_OFFSET64; /* Reset when GDR is written. */
    CDF->readOnly = FALSE;
    CDF->zMode = zMODEoff;
    ...
    **************************************************************************/
    if (!CACHEv64(CDF->fp,CDF->workingCacheSize)) {
      ...
      cdf_FreeMemory (CDF->CDFname, NULL);
      cdf_FreeMemory (CDF, NULL);
    ...
    **************************************************************************/
    ...
    GDR.RecordSize = GDR_BASE_SIZE64 + (OFF_T) (numDims * 4);
    GDR.RecordType = GDR_;
  ...
  ****************************************************************************/
  ...
  case zVAR_: {
    ...
    * Write rVDR/zVDR.
    **************************************************************************/
    if (zOp) ttmmpp = zNumDims * 4;
    else ttmmpp = 0;
    VDR.RecordSize = BOO(zOp,zVDR_BASE_SIZE64,rVDR_BASE_SIZE64) +
		     ttmmpp +                                     /*DimSizes.*/
		     BOO(zOp,zNumDims, CDF->rNumDims)*4;	  /*DimVarys.*/
