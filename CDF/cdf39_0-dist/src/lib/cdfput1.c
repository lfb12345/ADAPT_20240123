  ...
  ****************************************************************************/
  ...
  case zVAR_BLOCKINGFACTOR_: {
    ...
    **************************************************************************/
    ...
    if (zOp) {
      if (!sX(ReadVDR(CDF,CDF->fp,Var->VDRoffset64,Var->zVar,
      ...
      if (!sX(ReadGDR(CDF->fp,CDF->GDRoffset64,
...
/*      sX (FORCED_PARAMETER, &pStatus); */
    ...
    if (VARcompressionBITset(flags) && RECvaryBITset(flags) && (blocking != 0))
    {
      ...
      for (i = 0; i < (int) numDims; ++i)
        if (dimVarys[i]) recSize *= dimSizes[i];
      total = (long) blocking * recSize;
      if (total < MIN_BLOCKING_BYTES_compressed) {
      }
      if (total > (long) NUMcacheSTAGE*MIN_BLOCKING_BYTES_compressed) {
	int numCaches, iy;
  ...
  ****************************************************************************/
  ...
  case zVAR_DATA_: {
    ...
    * Write value.
    **************************************************************************/
    if (zOp != CDF->keptVarzOp || Var->varN != CDF->keptVarNum) {
      /* Not reading the same variable... clearing the caches if the    */
      /* remaining caches can't hold the new compressed/sparse data to  */
      /* avoid filling up all caches and then a temporary file becomes  */
      /* necessary.             */
      if (CDF->stage.fp != NULL && (COMPRESSED(Var) || SPARSErecords(Var)) &&
	  CheckForOverCaches(CDF, Var)) {
	if (!sX(FlushAllVarStages(CDF,Var),&pStatus)) return pStatus;
	V_freecaches (CDF->stage.fp);
	V_stagereset (CDF);
      }
      CDF->keptVarzOp = zOp;
      CDF->keptVarNum = Var->varN;
    }
  ...
  ****************************************************************************/
  ...
  case zVAR_HYPERDATA_: {
    ...
    **************************************************************************/
    /**************************************************************************
    * Write values.
    * Delete the staging vFile to free up the caches so the new write can
    * start from the scratch, if the variable is chaned.
    **************************************************************************/
    if (zOp != CDF->keptVarzOp || Var->varN != CDF->keptVarNum) {
      if (CDF->stage.fp != NULL && (COMPRESSED(Var) || SPARSErecords(Var)) &&
	  CheckForOverCaches(CDF, Var)) {
