...
STATICforIDL CDFstatus DecompressCDF (CDF, dotFp, uDotFp)
...
vFILE *uDotFp;          /* In: Uncompressed CDF file pointer. */
{
  ...
  ****************************************************************************/
  ...
  if (!sX(Decompress(CDF, dotFp,cOffset,
...
Logical empty;          /* In: If TRUE, write an empty CCR. */
{
  ...
  ****************************************************************************/
  if (empty) {
    ...
    cOffset = V2_CCR_OFFSET + CCR_BASE_SIZE;
    if (!sX(Compress(CDF,CDF->uDotFp,FIRST_IR_OFFSET,
  ...
  if (Var->EncodeFunction == NULL) {
    ...
    if (nBytes <= REWR_MAX2) {
      if (STRINGdataType(Var->dataType)) {
        ...
        if (!WRITEv(newBuffer,1,nBytes,fp)) {
          cdf_FreeMemory (newBuffer, NULL);
          return VAR_WRITE_ERROR;
        }
        cdf_FreeMemory (newBuffer, NULL);
      } else
        if (!WRITEv(buffer,1,nBytes,fp)) return VAR_WRITE_ERROR;
    } else {
	...
	toBytes = (remaining>mBytes?(size_t)mBytes:(size_t)remaining);
        if (!WRITEv(tBuffer+i*mBytes,1,toBytes,fp))
          return VAR_WRITE_ERROR;
        remaining -= toBytes;
        ++i;
      }
...
void *buffer;
{
  CDFstatus pStatus = CDF_OK; size_t nBytes, mBytes;
  /****************************************************************************
  * Seek to the desired offset.
  ****************************************************************************/
  ...
  ****************************************************************************/
  ...
  if (nBytes <= REWR_MAX2) {
    ...
    size_t toBytes; Int32 toElems;
    Byte1 *tBuffer = (Byte1 *) buffer;
    size_t remaining = nBytes;
    int i = 0;
    toElems = (Int32) REWR_MAX2 / Var->NelemBytes;
    mBytes = (size_t) toElems * Var->NelemBytes;
    while (remaining > 0) {
      toBytes = (remaining>mBytes?(size_t)mBytes:(size_t)remaining);
      if (!READv(tBuffer+i*mBytes,toBytes,1,fp))
  ...
  switch (Var->vType) {
    ...
    case SPARSE_COMPRESSED_RECORDS_: {
      ...
      ************************************************************************/
      ...
      if (!sX(Compress(CDF,CDF->stage.fp,Var->stage.areaOffset,
...
VISIBLE_PREFIX CDFstatus GetFILLVALEntry (id, Z, varNum, dataType, numElems,
					  value)
CDFid id;
Logical Z;
long varNum;
