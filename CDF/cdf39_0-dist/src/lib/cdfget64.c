...
switch (Va->item) {
  ...
  ****************************************************************************/
  ...
  case zVAR_ALLOCATEDFROM_: {
    ...
    if (!CDF->readOnly) {
      ...
      /************************************************************************
      * If the compressed or sparse records, flush out the variable if its
      * VXR is not yet created.
      ************************************************************************/
      if (VARcompressionBITset(flags) || sRecords != NO_SPARSERECORDS) {
  ...
  ****************************************************************************/
  ...
  case zVAR_ALLOCATEDTO_: {
    ...
    if (!CDF->readOnly) {
      ...
      /************************************************************************
      * If the compressed or sparse records, flush out the variable in case
      * its VXR is not yet created.
      ************************************************************************/
      if (VARcompressionBITset(flags) || sRecords != NO_SPARSERECORDS) {
        struct VarStruct *Var;
        if (zModeON(CDF))
  ...
  ****************************************************************************/
  ...
  case zENTRY_DATA_: {
    ...
    SelectCDF (Cur->cdf, CDF)
    if (!CURRENTattrSELECTED64(CDF)) return NO_ATTR_SELECTED;
    if (E3(entryType,CDF->CURgrEntryNum,
		     CDF->CURgrEntryNum,
