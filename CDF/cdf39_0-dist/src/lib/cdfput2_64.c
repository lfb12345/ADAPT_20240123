...
switch (Va->item) {
  ...
  ****************************************************************************/
  case CDF_COMPRESSION_: {
	...
	*   -- dotFp is compressed
	*   -- uDotFp is uncompressed (from dotFp)
	*   1. delete dotFp
	*   2. re-create dotFp
	*   3. set dotFp's cache size
	*   4. copy uDotFp to dotFp
	*   5. flush dotFp
	*   6. delete uDotFp
	**********************************************************************/
	if (CDF->uDotFp != NULL) {
	...
	**********************************************************************/
	if (!WriteAccess64(CDF,FALSE,&pStatus)) return pStatus;
	/**********************************************************************
	* If the CDF is not currently compressed (uDotfp is NULL otherwise
	* cdfope will create one), create an uDotfp
	* file and copy the current contents into it.
	*   -- dotFp is not compressed (as uDotFp does not exist)
	*   -- uncompressed CDF to compressed CDF --
	*   1. make uDotFp as a scratch
	*   2. set uDotFp's cache size
	*   3. copy dotFp to uDotFp
	*   4. flush uDotFp --- creating a temporary file (actully a CDF - old)
	*   5. delete dotFp
	*   6. recreate dotFp
	*   7. write compressed CDF to dotFp
	*   8. flush dotFp 
	**********************************************************************/
	if (CDF->uDotFp == NULL) {
	  ...
	  CDF->workingCacheSize = NUMcacheCOMPRESS;
	...
	**********************************************************************/
	...
	CPR.RecordSize = CPR_BASE_SIZE64 + (pCount * 4);
	CPR.RecordType = CPR_;
	CPR.rfuA = 0;
  ...
  ****************************************************************************/
  ...
  case zVAR_COMPRESSION_: {
    ...
    **************************************************************************/
    ...
    /**************************************************************************
    * If the existing blocking factor is too small/large, then give warning.
    **************************************************************************/
    if (!zOp) {
	...
	if (VDR.DimVarys[i]) recSize *= (zOp?VDR.zDimSizes[i]:dimSizes[i]);
      total = (OFF_T) recSize * VDR.blockingFactor;
      if (total < (long) MIN_BLOCKING_BYTES_compressed) {
      }
      if (total > (OFF_T) NUMcacheSTAGE * MIN_BLOCKING_BYTES_compressed) {
	int numCaches, iy; Int32 blocking;
	numCaches = (int) ((total-1) / MIN_BLOCKING_BYTES_compressed) + 1;
	blocking = VDR.blockingFactor;
	iy = 0;
	while (numCaches > NUMcacheSTAGE) {
	  ...
	  total = (OFF_T) recSize * blocking;
	  numCaches = (int) ((total-1) / MIN_BLOCKING_BYTES_compressed) + 1;
	}
	if (iy == 0)
	  sX (BLOCKINGFACTOR_TOO_LARGE, &pStatus);
	VDR.blockingFactor = blocking;
      }
    }
    /**************************************************************************
  ...
  ****************************************************************************/
  ...
  case zENTRY_DATA_: {
    ...
    if (!STRINGdataType(dataType) && numElems < 1L) return BAD_NUM_ELEMS;
    if (STRINGdataType(dataType) && numElems < 1L) {
      if ((*((unsigned char *)value)) == '\0') numElems = 1L;
      else return BAD_NUM_ELEMS;
    }
    ...
    if (STRINGdataType(dataType)) {
      int cnt;
/*
      if (UTF8Check((unsigned char *)value, (int) numElems, &cnt) != NULL)
	pStatus = STRING_NOT_UTF8_ENCODING;
*/
    }
