  ...
  switch (Var->vType) {
    ...
    case SPARSE_RECORDS_: {
      if (Var->firstRecInVVR <= phyRecN && phyRecN <= Var->lastRecInVVR) {
        *offsetP = Var->offsetOfVVR64 + (OFF_T) (VVR_BASE_SIZE64 +
                   ((OFF_T) Var->NphyRecBytes) * (phyRecN -
                                                  Var->firstRecInVVR));
	break;
      }
      else {
...
*/
  ...
  ****************************************************************************/
  if (read_only_mode == READONLYon)
  {
      ...
      if (searchNum < 0 || searchNum >= CDF->fp->GDR64->NumAttr)
	return NO_SUCH_ATTR;
...
OFF_T *offset;          /* Out: offset of the VDR. */
{
  ...
  * variables are accessed in an orderly sequence.
  ****************************************************************************/
  for (varN = 0; varN < nVars; varN++) {
...
void *buffer;                   /* Buffer to pad. */
{
  ...
  CDFstatus status;
  void *filled = NULL; long dataType2, numElems2;
  ...
  if (status == CDF_OK) {
    ...
    **************************************************************************/
    if (PADvalueBITset(flags)) {
      ...
      if (!sX(DefaultPadBuffer64(CDF,Var,nValues,buffer),&pStatus))
        return pStatus;
...
STATICforIDL CDFstatus Compress64 (CDF, iFp, iOffset, iSize, iError, cType,
				   cParms, oFp, oOffset, oSize, oError)
struct CDFstruct *CDF;
vFILE *iFp;
OFF_T iOffset;
OFF_T iSize;
...
STATICforIDL CDFstatus Decompress64 (CDF, iFp, iOffset, iSize, iError, cType,
				     cParms, oFp, oOffset, oError)
struct CDFstruct *CDF;
vFILE *iFp;
OFF_T iOffset;
OFF_T iSize;
...
OFF_T uSize;
{
  ...
  switch (irType) {
    ...
    case CVVR_: {
      ...
      if (!sX(Decompress64(CDF,CDF->fp,tOffset,
