...
*   V3.26 18-Jan-23, M Liu      Changed nCACHE_BUFFER_BYTEs for Unix/Cygwin
*                               to 131072 (128k).
*
******************************************************************************/
...
#define BeginUnixTimeEPOCH	62167219200000.0
#define BeginUnixTimeEPOCH16	62167219200.0
...
******************************************************************************/
    ...
    defined(__PPC__) || defined(__ppc64__) || defined(ARM_BIG) 
#  define NETWORKbyteORDERcpu
#endif

/******************************************************************************
* XDRishENCODINGcpu.
*   xdr...........Sun, SGi, IBM-RS, HP, NeXT, Macintosh, PowerPC, ARM_BIG
...
/*****************************************************************************
* caches:
*   Initial number of cache buffers.
*   Byte count of each cache buffer.
*****************************************************************************/
#define DEFAULT_nCACHE_BUFFERs  1               /* Default number of buffers
						   in the cache. */
#if defined(win32) && (_FILE_OFFSET_BITS == 64) 
#    define nCACHE_BUFFER_BYTEs     65536     /* Bytes of each buffer */
#else
#  if defined(linux) || defined(unix) || defined(__CYGWIN__)
#    define nCACHE_BUFFER_BYTEs     131072 
#  else
#    define nCACHE_BUFFER_BYTEs     65536
#  endif
#endif
...
/******************************************************************************
 * FLOAT4dataType.
 *    Macro to check for 4-byte floating point data types.
 ******************************************************************************/

#define FLOAT4dataType(dataType) \
(dataType == CDF_REAL4 || dataType == CDF_FLOAT)

/******************************************************************************
 * DOUBLEdataType.
 *    Macro to check only non-epoch 8-byte floating point data types.
 ******************************************************************************/

#define DOUBLEdataType(dataType) \
(dataType == CDF_REAL8 || dataType == CDF_DOUBLE)

/******************************************************************************
* FLOAT8dataType.
*    Macro to check for all double-precision floating-point data types.
******************************************************************************/
...
#define FREECACHESv(fp) (V_freecaches(fp))
...
typedef struct vFILEstruct {
  ...
  long *cacheNumInd;    /* Cache number indicators for a scratch/compressed
			   CDF file. */
  vCACHE **cacheLocPtr; /* Cache pointers for a scratch/compressed CDF file. */
...
struct AEDRstruct {
  ...
  Int32 NumStrings;     /* Number of strings of string data type */
  Int32 rfuB;           /* reserved for future use (value = 0) */
  Int32 rfuC;           /* reserved for future use (value = 0) */
  Int32 rfuD;           /* reserved for future use (value = -1) */
  Int32 rfuE;           /* reserved for future use (value = -1) */
...
#define AEDR_NUMSTRINGS_OFFSET  28
...
struct CDFstruct {
  ...
  vFILE *compressFp;			/* Compression scratch file pointer.
					   ...
					   file has not been created yet. Used
					   for creating a CDF with compression  
					   at variable-level. */
  ...
  vSTATS uDotCDFvStats;			/* Vstream statistics for the
					   ...
					   compressed CDF). */
  long  checksum;			/* Indicator for checksum. */
...
STATICforIDL CDFstatus DecompressCDF PROTOARGs((struct CDFstruct *CDF,vFILE *dotFp, vFILE *uDotFp));
...
STATICforIDL CDFstatus Compress PROTOARGs((
  struct CDFstruct *CDF, vFILE *iFp, Int32 iOffset, Int32 iSize, CDFstatus iError, Int32 cType,
...
STATICforIDL void FillNulsToString PROTOARGs((char *buffer, int totalBytes,
  int numElems
));
...
VISIBLE_PREFIX void V_stagereset PROTOARGs((struct CDFstruct *CDF));
...
VISIBLE_PREFIX Logical CheckForOverCaches PROTOARGs((
  struct CDFstruct *CDF, struct VarStruct *Var
));
VISIBLE_PREFIX CDFstatus FlushAllVarStages PROTOARGs((
  struct CDFstruct *CDF, struct VarStruct *Var
));
#if !defined(__CFM68K__) || defined(__USING_STATIC_LIBS__) || !defined(CFM68KDLL)
...
VISIBLE_PREFIX void V_freecaches PROTOARGs((vFILE *vFp));
...
VISIBLE_PREFIX CDFstatus GetFILLVALEntry PROTOARGs((
  CDFid id, Logical zVar, long varNum, long *dataType, long *numElms,
  void **value
));
VISIBLE_PREFIX Logical EquivalentDataTypes PROTOARGs((long, long));
VISIBLE_PREFIX void ShowMemoryAllocCounts PROTOARGs((void));
#if defined(MSVC67)
