...
print,'SPASE_KEYWORD_LIST'

print,''

help,spase_keyword_list,/structure
...
;      REQUIRED MASKING!!!
       ...
       help,structure_name,group_incl_num

       print,''

       print,spase_ontology(structure_start_line).group_incl
       ...
       if (group_incl_num gt 0) then begin
          ...
          for group_incl_index=0,group_incl_num-1 do begin
              ...
              help,case_char,group_incl_index
              ...
              if (case_char ne spase_ontology(structure_start_line).group and $
                 ...
                 keyword_list=make_array(spase_keyword_list(case_index).num, $
                                         /string,value='')

                 for keyword_index=0,spase_keyword_list(case_index).num-1 do begin

                     keyword_list(keyword_index)= $
                        spase_keyword_list(case_index).keyword(keyword_index)

                     print,keyword_index,keyword_list(keyword_index)

                 endfor

                 keyword_list= $
                    keyword_list(uniq(keyword_list,sort(keyword_list)))

                 keyword_list_num=n_elements(keyword_list)

                 for keyword_index=0,keyword_list_num-1 do $
                     suffix=suffix+',keyword_'+keyword_list(keyword_index)+ $
                            '=keyword_'+keyword_list(keyword_index)
